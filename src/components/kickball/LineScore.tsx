    ...
    <table className="w-full table-fixed border-collapse overflow-hidden rounded-xl text-center text-[11px] font-bold tabular-nums">
      <thead>
        <tr className="bg-muted text-muted-foreground">
          ...
          <th className="px-0 py-0.5 font-black uppercase text-primary">R</th>
        </tr>
      </thead>
      <tbody>
        {rows.map((row) => (
          <tr key={row.team} className="border-t border-border">
            <td className="max-w-0 truncate px-1 py-0.5 text-left text-[10px] font-black uppercase tracking-wide text-foreground">
              {row.name}
            </td>
            {row.runs.map((runs, i) => (
              <td
                ...
                className={cn(
                  "p-0",
                  row.active && i === inning - 1 && "bg-primary/20 ring-1 ring-inset ring-primary",
                )}
              >
                <input
                  type="number"
                  ...
                  className="h-6 w-full [appearance:textfield] border-0 bg-transparent text-center text-[11px] font-bold text-foreground outline-none focus-visible:ring-1 focus-visible:ring-primary [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none"
                />
              </td>
            ))}
